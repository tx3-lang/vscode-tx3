...
const getIconUrl = (type: PartyType): string =>
  new URL(
    `../images/${type === "unknown" ? "party" : type}.svg`,
    import.meta.url,
  ).href;
...
const TxDiagram: React.FC<TxDiagramProps> = (props: TxDiagramProps) => {
  ...
  for (const [outputIndex, output] of outputs.entries()) {
    if (output.party) {
      const partyIndex = outputParties.findIndex(
        (p) => p.name === output.party,
      );
      if (partyIndex !== -1) {
  ...
  return (
    <svg
      width="100%"
      viewBox={`0 0 ${CANVA_WIDTH} ${CANVA_HEIGHT}`}
      className="my-16"
    >
      <Tx x={CANVA_WIDTH / 2} y="0" tx={props.tx} />
      {inputParties.map((party, index) => (
        <Party x="0" y={UNIT * index} party={party} key={index} />
      ))}
      ...
      {inputConnections.map(([partyIndex, inputIndex], index) => (
        <line
          key={index}
          x1={UNIT}
          y1={UNIT * partyIndex + UNIT / 2}
          x2={CANVA_WIDTH / 4 - UNIT / 8}
          y2={UNIT * (inputIndex + 1) - UNIT / 16}
          stroke="#fff"
          strokeWidth="0.4"
          strokeDasharray="1,1"
          strokeOpacity="0.5"
      ...
      {outputConnections.map(([partyIndex, outputIndex], index) => (
        <line
          key={index}
          x1={CANVA_WIDTH / 2 + CANVA_WIDTH / 4 + UNIT / 8}
          y1={UNIT * (outputIndex + 1) - UNIT / 16}
          x2={CANVA_WIDTH - UNIT}
          y2={UNIT * partyIndex + UNIT / 2}
          stroke="#fff"
          strokeWidth="0.4"
          strokeDasharray="1,1"
          strokeOpacity="0.5"
